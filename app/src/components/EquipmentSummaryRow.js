 ...
 *
 * The compact row shown for each piece of Equipment in the main list -
 * same pattern as DevikinSummaryRow.js and WeaponSummaryRow.js: a
 * picture on the left, a few key identity fields stacked on the right,
 * tappable to open the full detail card (see NftCard.js's equipment
 * layout, and CollectionView.js for the list/detail switch).
 *
 * Rarity / Type / Quality again, for consistency with Weapons - this is
 * a first pass, easy to swap once you've had a chance to think about
 * which fields you actually want to see at a glance here.
...
export default function EquipmentSummaryRow({ nft, onPress, selected = false }) {
  const { colors } = useTheme();

  // Same "prefer the locally-saved copy" logic as the other summary rows
  // and NftCard.js - see src/api/imageStorage.js for why.
